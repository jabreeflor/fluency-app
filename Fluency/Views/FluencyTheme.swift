import SwiftUI

enum FluencyTheme {
    // MARK: - Colors
    static let primary = Color(hex: "58CC02")       // Duolingo green
    static let primaryDark = Color(hex: "46A302")
    static let accent = Color(hex: "FF4B4B")        // hearts / danger
    static let gold = Color(hex: "FFC800")          // XP / streak
    static let surface = Color(hex: "F7F7F7")
    static let cardBg = Color.white
    static let textPrimary = Color(hex: "3C3C3C")
    static let textSecondary = Color(hex: "777777")
    static let border = Color(hex: "E5E5E5")

    // Exercise state colors
    static let correctGreen = Color(hex: "D7FFB8")
    static let correctBorder = Color(hex: "58CC02")
    static let wrongRed = Color(hex: "FFD0D0")
    static let wrongBorder = Color(hex: "FF4B4B")

    // MARK: - Typography
    static let titleFont = Font.system(.title, design: .rounded, weight: .bold)
    static let headlineFont = Font.system(.headline, design: .rounded, weight: .bold)
    static let bodyFont = Font.system(.body, design: .rounded)
    static let captionFont = Font.system(.caption, design: .rounded)

    // MARK: - Sizing
    static let cornerRadius: CGFloat = 12
    static let buttonHeight: CGFloat = 52
    static let cardPadding: CGFloat = 16
}

// MARK: - Color hex init

extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default: (a, r, g, b) = (255, 0, 0, 0)
        }
        self.init(.sRGB, red: Double(r)/255, green: Double(g)/255, blue: Double(b)/255, opacity: Double(a)/255)
    }
}

// MARK: - Reusable Button Style

struct FluencyButtonStyle: ButtonStyle {
    var color: Color = FluencyTheme.primary
    var isDisabled: Bool = false

    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(FluencyTheme.headlineFont)
            .foregroundColor(.white)
            .frame(maxWidth: .infinity, minHeight: FluencyTheme.buttonHeight)
            .background(isDisabled ? Color.gray.opacity(0.4) : (configuration.isPressed ? color.opacity(0.8) : color))
            .clipShape(RoundedRectangle(cornerRadius: FluencyTheme.cornerRadius))
            .scaleEffect(configuration.isPressed ? 0.97 : 1.0)
            .animation(.easeOut(duration: 0.1), value: configuration.isPressed)
    }
}

struct FluencyOutlineButtonStyle: ButtonStyle {
    var color: Color = FluencyTheme.primary

    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(FluencyTheme.headlineFont)
            .foregroundColor(color)
            .frame(maxWidth: .infinity, minHeight: FluencyTheme.buttonHeight)
            .background(.clear)
            .overlay(
                RoundedRectangle(cornerRadius: FluencyTheme.cornerRadius)
                    .stroke(color, lineWidth: 2)
            )
            .scaleEffect(configuration.isPressed ? 0.97 : 1.0)
            .animation(.easeOut(duration: 0.1), value: configuration.isPressed)
    }
}
